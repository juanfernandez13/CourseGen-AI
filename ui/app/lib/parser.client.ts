...
import { ommlToLatex } from '../../server/ommlToLatex';

type XmlNode =
  | { type: 'element'; name: string; attrs: Record<string, string>; children: XmlNode[] }
  | { type: 'text'; value: string };

export async function processMatriz(file: File | Blob): Promise<string> {
...
function extractTextWithMath(xml: string): string {
  ...
  const walk = (node: XmlNode) => {
    ...
    if (name === 'm:oMathPara') {
      const latex = (ommlToLatex(node) as string).trim();
      if (latex) {
        flush();
        lines.push(`\\[ ${latex} \\]`);
      }
...
function parseXml(xml: string): XmlNode {
  ...
  const root: XmlNode = { type: 'element', name: '#root', attrs: {}, children: [] };
  const stack: Extract<XmlNode, { type: 'element' }>[] = [root as Extract<XmlNode, { type: 'element' }>];
  ...
  while (i < len) {
    ...
    const node: Extract<XmlNode, { type: 'element' }> = { type: 'element', name, attrs, children: [] };
